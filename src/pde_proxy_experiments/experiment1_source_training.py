"""
Experiment 1: Source Training Validation

Train Physics Operator Network on Burgers 1D equation with ν=0.01/π (source domain).

Objective:
- Validate that two-network architecture can solve Burgers equation accurately
- Establish baseline accuracy for comparison
- Train physics operator F_φ that will be frozen for transfer

Success Criteria:
- L2 relative error < 2% (competitive with traditional PINN)
...
from burgers_physics_operator import BurgersPhysicsOperatorPINN


class BurgersDataLoader:
    """Load and preprocess Burgers 1D reference data from PINNacle"""

    def __init__(self, datapath="/workspace/PINN4SOH/src/PINNacle/ref/burgers1d.dat"):
    ...
    def load_data(self):
        ...
        # First column is x coordinates
        self.x_ref = data[:, 0]  # Shape: (101,)

        # Remaining columns are u at different time steps
        # Columns: u @ t=0, t=0.1, t=0.2, ..., t=1.0 (11 time steps)
        self.u_ref = data[:, 1:]  # Shape: (101, 11)

        # Time coordinates
        self.t_ref = np.linspace(0, 1, 11)  # 11 time steps

        print(f"Loaded Burgers 1D reference data:")
        print(f"  Spatial points: {len(self.x_ref)}")
        print(f"  Temporal points: {len(self.t_ref)}")
        print(f"  X range: [{self.x_ref.min():.2f}, {self.x_ref.max():.2f}]")
        print(f"  T range: [{self.t_ref.min():.2f}, {self.t_ref.max():.2f}]")
        print(f"  U range: [{self.u_ref.min():.4f}, {self.u_ref.max():.4f}]")

    def get_ic_data(self, n_points=100):
        """Get initial condition data at t=0"""
    ...
    def get_bc_data(self, n_points=100):
        """Get boundary condition data: u(-1, t) = u(1, t) = 0"""
    ...
    def get_collocation_data(self, n_points=1000):
        """Get random collocation points for physics loss"""
        x = np.random.uniform(-1, 1, (n_points, 1))
        t = np.random.uniform(0, 1, (n_points, 1))
        return x, t

    def get_test_data(self):
        """Get test data from reference solution"""
        # Create meshgrid
        X, T = np.meshgrid(self.x_ref, self.t_ref)
        x_test = X.flatten()[:, None]
        t_test = T.flatten()[:, None]
        u_test = self.u_ref.T.flatten()[:, None]
...
def train_source_domain(
    ...
    """
    Train Physics Operator Network on source domain (ν = 0.01/π)

    Loss = λ_ic * Loss_IC + λ_bc * Loss_BC + λ_physics * Loss_physics

    Where:
    - Loss_IC: Initial condition loss ||u(x,0) - sin(-πx)||²
    - Loss_BC: Boundary condition loss ||u(±1,t) - 0||²
    - Loss_physics: Physics loss ||F_φ(u, u_x, u_xx) - u_t||²
    """
    ...
    scheduler_sol = optim.lr_scheduler.ReduceLROnPlateau(
        optimizer_sol, mode='min', factor=0.5, patience=1000, verbose=True
    )
    scheduler_phys = optim.lr_scheduler.ReduceLROnPlateau(
        optimizer_phys, mode='min', factor=0.5, patience=1000, verbose=True
    )
    ...
    # Get test data once
    x_test, t_test, u_test = data_loader.get_test_data()
    ...
    print(f"\nStarting source domain training...")
    ...
    for epoch in pbar:
        ...
        x_ic, t_ic, u_ic = data_loader.get_ic_data(n_ic)
        x_bc, t_bc, u_bc = data_loader.get_bc_data(n_bc)
        x_col, t_col = data_loader.get_collocation_data(n_collocation)
...
if __name__ == "__main__":
    ...
    # Initialize model
    nu_source = 0.01 / np.pi
    model = BurgersPhysicsOperatorPINN(
        nu=nu_source,
    ...
    # Load data
    data_loader = BurgersDataLoader()

    # Train (reduced epochs for proof-of-concept)
    save_dir = Path("results/experiment1_source_training")
    history = train_source_domain(
        model=model,
        data_loader=data_loader,
        n_epochs=5000,  # Reduced from 20000 for faster proof-of-concept
        n_ic=100,
        n_bc=100,
        n_collocation=2000,
        lr_solution=1e-3,
    ...
    print("\n" + "="*60)
    print("EXPERIMENT 1: SOURCE TRAINING - COMPLETE")
    print("="*60)
