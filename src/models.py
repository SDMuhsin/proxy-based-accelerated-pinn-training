"""
PINN Model Components for Battery SOH Prediction

This module contains the neural network architectures for Physics-Informed Neural Networks (PINN)
including the solution network, physics constraint network, and training utilities.
"""
...
from src.utils.logging import AverageMeter, get_logger, eval_metrix
import os
...
class Sin(nn.Module):
    """Sine activation function for physics-informed networks"""
...
class MLP(nn.Module):
    """Multi-layer Perceptron with sine activation functions"""
...
class Predictor(nn.Module):
    """Prediction head for SOH estimation"""
...
class Solution_u(nn.Module):
    """Solution network: maps input features to SOH prediction"""
...
class PINN(nn.Module):
    """Physics-Informed Neural Network for battery SOH prediction"""
    ...
    def Train(self, trainloader, testloader=None, validloader=None):
        ...
        for e in range(1, self.args.epochs + 1):
            ...
            info = '[Train] epoch:{}, lr:{:.6f}, total loss:{:.6f}'.format(
                e, current_lr, loss1 + self.alpha * loss2 + self.beta * loss3)
            ...
            if valid_mse < min_valid_mse and testloader is not None:
                ...
                info = '[Test] MSE: {:.8f}, MAE: {:.6f}, MAPE: {:.6f}, RMSE: {:.6f}'.format(
                    MSE, MAE, MAPE, RMSE)
                self.logger.info(info)
                early_stop = 0
